...
{                                              /*use long in case of a 64−bit system */
    ...
    for (int i = 0; i < thread_count; i++){
        ...
        for (int level = 0; level < process_n; level++){
            for (int indx = 0; indx < ((int) thread_count); indx += (int) pow(2, level)){
                ...
                // dest = indx - (indx % (int)pow(2, level +1)) * ( pow(2, level));
        ...
        my_size[i] = (int) size* (n/thread_count);
    }

    local_array = malloc(my_size[my_rank] * sizeof(int));
    ...
    for (int level = 0; level < process_n; level++){
        for (int indx = 0; indx < ((int) thread_count); indx += (int) pow(2, level)){
            ...
            if ((my_rank == dest) && (dest != indx)){
                ...
                temp = malloc(size_of_new_array * sizeof(local_array[0]));
                MPI_Recv(temp, size_of_new_array, MPI_INT, indx, 2, MPI_COMM_WORLD, &status);

                long long size_of_temp_local = size_of_local_array + size_of_new_array;

                temp_local = malloc(my_size[my_rank] * sizeof(int));
                ...
                memcpy(local_array, temp_local, my_size[my_rank]*sizeof(int));
    ...
    if(my_rank == 0){
        ...
        int * serial_array;
        serial_array = malloc(n * sizeof(int));
